...
**Benefits.** The five artifacts, once finished, keep three things apart that a record usually
blurs: a control you run, one you dropped on purpose, and one nobody has reached.

**Costs.** Less at the start than you expect. A team that reviews changes, runs a linter, keeps
secrets out of the repository and pins some dependencies already satisfies parts of four of these
documents. Nothing here ships as code: every control these documents name is one you would build.
At least five of those controls can never be automated, and stay with a person.
...
| Secrets and restricted data kept out of the repository and its full history, backed by a fail-closed commit-time content scan | [Secure development](SECURE-DEVELOPMENT.md), SD-6.1 and SD-6.2; [the leak gate](https://claude-multisession.pages.dev/LEAK-GATE.html) | | | |
...
so instrumentation comes before ranking ([Case study: a drift audit](https://claude-multisession.pages.dev/CASE-STUDY-drift-audit.html),
...
[the leak gate](https://claude-multisession.pages.dev/LEAK-GATE.html); read them there.
...
([Here's what to feed to your AI coding assistant](https://claude-multisession.pages.dev/FEED-THIS-TO-CLAUDE-CODE.html), *"If they decide
to proceed"*).
...
template](https://claude-multisession.pages.dev/CLAUDE.md.template),
...
| Where a project keeps its standing rules, and a table for what is live | [The `CLAUDE.md` template](https://claude-multisession.pages.dev/CLAUDE.md.template) |
...
| A fail-closed content scanner and the blind spot no scanner closes | [The leak gate](https://claude-multisession.pages.dev/LEAK-GATE.html) |
| The same reasoning applied to one repository's own controls | [Case study: a drift audit](https://claude-multisession.pages.dev/CASE-STUDY-drift-audit.html) |
