...
**Benefits.** Inbound, every change arrives pinned, surveilled and reviewable rather than silent.
Outbound, the release carries its own inventory and provenance, so an adopter checks what you
shipped instead of emailing you for it.

**Costs.** The recurring cost is a few minutes of diligence per new dependency, a review per
version bump, and one wiring exercise per manifest. This document ships no configuration -- no
workflow, no lockfile, no publishing pipeline. None of this detects a
[malicious release from a legitimate maintainer](#costs-and-where-it-does-not-apply).
...
### The inventory is a published artifact, and the buyer calls it an SBOM

What this document calls a component inventory, a procurement questionnaire calls a **software bill
of materials**. Same artifact, two vocabularies, and the second is the one you will be asked for in
writing.

Three questions are answered on the site already and are not restated here:

* which regimes ask for one
* which machine-readable formats are current
* what the published minimum-element sets do and do not require

They are in [the standards reference](STANDARDS-REFERENCE.md). Three properties decide whether the
artifact answers anything, and none of them are the format.

**Rule.** Publish one inventory **per released artifact**, generated by **the build that produced
that artifact**, and serve it wherever the artifact is served.

**Per released artifact, not per project.** A repository-level inventory describes a moving target.
The question an inventory exists to answer is version-shaped -- *what is in the version this adopter
is running* -- which is the same question the release archive answers, one section down. An inventory
not pinned to a released version cannot answer it, and reads as though it can.

**Generated by the build, never hand-maintained.** A hand-written list is accurate on the day it is
written and silently wrong afterwards. It is the exclusion-list failure with the sign flipped: an
exclusion list ships whatever nobody thought to exclude, a hand-maintained inventory omits whatever
nobody thought to add. Both are right about everything except the thing that changed.

**Served, not available on request.** An inventory a buyer has to email you for is a support process
rather than an artifact. Attaching it to the release costs nothing per release once wired, and it
takes you out of the loop on the day an advisory lands and every adopter asks the same question at
once.

**What it establishes, and the line past which it establishes nothing.** An inventory states **what
is in the build**. It does not state that any of it is **exploitable in a given configuration**, and
the gap between those two sentences is where most inventory-driven effort is wasted. Whether a listed
component is a risk depends on whether the affected code path is built in at all, whether it is
invoked, whether it is reachable given the deployment's configuration, and whether an attacker
controls an input that reaches it. A manifest produced by a tool that read the build carries none of
that and cannot.

That answer is a **separate, per-advisory statement**: for one advisory against one listed component,
whether this product is affected and why. Machine-readable vocabularies for it exist -- the one in
general circulation is an exploitability statement, or VEX -- and what a consumer reads out of one is
a status per advisory per product, plus the reason behind that status. Its author is you, not the
tooling, which is exactly why it is the scarce artifact and the inventory is not.

This is the statement you make about somebody else's advisory against a component you shipped. The
advisory you issue about a defect of your own runs the other direction and is section 12 of
[secure development](SECURE-DEVELOPMENT.md); the two are adjacent, both are configuration-conditional
claims about exploitability, and they are not the same document.

**Rule.** Label which of the two you are handing over. Never let an inventory entry be read as an
exposure, and never let an absence be read as safety -- an inventory is only as complete as the build
it was generated from, and anything that entered the running system after the build is not in it at
all.

### If you host the software, the useful inventory is of the runtime

This distinction is the one most SBOM advice skips, and it changes which document is the right one to
hand over.

For **software you distribute**, a build-time inventory of the released artifact is exactly right:
the adopter installs that artifact, so its inventory describes what they are running.

For **software you operate on their behalf**, it is the wrong document. What is running is a deployed
environment:

* base images
* operating-system packages
* sidecar and agent processes
* whatever was patched in place since the build
...
buyer who asked a hosted service for an SBOM and received a build manifest has not learned what is
running, and will usually not notice.
...
| Component inventory per **released artifact**, generated by the build, attached to the release and the archive | Universal (format choice is consumer-driven) |
| A hosted service publishes an inventory of the **runtime environment**, not only of the build | Universal in concept, deployment-specific in tooling |
...
| Releasing | One inventory per released artifact, generated by the build, served where the artifact is |
| Claiming | An inventory answers "do we ship X". It is not tamper detection |
| Claiming | An inventory states what is in the build, never what is exploitable in a configuration |
| Claiming | Publish the producer/operator responsibility split before claiming any control |
| Hosting | If you operate the software, publish an inventory of the runtime environment, not only of the build |
...
- **The inventory stays per-artifact and build-generated.** A hand-maintained one is correct until
  the first change nobody remembered to record, and it is correct-looking forever after.
...
- [The leak gate](https://claude-multisession.pages.dev/LEAK-GATE.html) -- fail-closed content scanning before publication, and the
...
- [Tips and tricks](https://claude-multisession.pages.dev/TIPS-AND-TRICKS.html) -- writing a guardrail, and measuring whether it works
- [Case study: auditing a multi-session estate as one system](https://claude-multisession.pages.dev/CASE-STUDY-drift-audit.html) --
