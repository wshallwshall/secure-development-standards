...
| `test_prose_rules_hold.py` | the three banned constructions in `docs/HOUSE-STYLE.md` that can be matched without judgment are absent from every tracked page, two measured qualities -- sentences over 30 words, table cells over 40 -- do not get worse than their baseline, and the rule sheet every one of those messages cites is present and carries a definition row for each rule the suite sends anyone to read | `HOUSE-STYLE.md` stated the writing rules and **nothing enforced any of them**, so the only thing between the corpus and a slow drift back was whether a reviewer remembered a rule identifier. The design constraint is what makes this file worth reading: hard-fail **only** where a violation is unambiguous, because a gate that reddens a legitimate editorial choice is one people delete -- a judgment this repository already recorded once, in `TheBlufConventionHasOneSpelling`. So every candidate was measured over the corpus and every hit was read before being enforced, and **B-7 was rejected on that evidence**: "cleanly" fires 16 times and not one is the self-praise the rule bans -- `git merges both cleanly`, `a session that exits cleanly`, `an alias stub that resolves cleanly`. The rule turns on *what is being described*, which no pattern over the text can see. `B-10` went the same way on its single hit. The scanner rejoins wrapped lines into paragraphs first, and that is not a detail: `HS-14` wraps prose near 100 characters, so a line-based scan reported **zero** sentences over 30 words across 110,000 words, and read a line beginning with a wrapped "importantly" as a sentence opener. Both were observed here before it was rewritten. `HOUSE-STYLE.md` must be able to quote what it bans, so a table row whose first cell is a rule identifier is exempt -- that row and nothing more, or the rule sheet becomes the one unchecked page in the repository. **The rule sheet itself then went missing and nothing said so.** It was cited in four docstrings, in the message a tripped developer reads, and in this row, while `git log --all --follow` over `docs/HOUSE-STYLE.md` returned nothing: it stayed in the toolkit repository at the split, so the reader sent to look up B-3 could not open the file. A citation in a failure message is prose to every checker here, which is why nothing caught it. `TheRuleSheetIsInThisRepository` now pins both halves -- the file exists, and every rule cited anywhere under `tests/` or `.github/workflows/` has a definition row in it. The exemption was measured while that was fixed, and it does one job rather than the one its name suggests: it keeps long definition rows off the fat-cell ratchet. The banned-construction scan never saw a table row at all, rule row or not, because the paragraph rejoin drops every line beginning with a pipe -- which leaves a banned construction inside an ordinary table cell caught by nothing, a gap recorded on the rule sheet rather than closed, since closing it means judging a cell by a sentence rule and `PD-4` forbids exactly that |
...
| `test_the_forwarding_site_covers_every_page.py` | the forwarding stubs that replace the old GitHub Pages content are derived from the markdown tree rather than curated, one per served page; the destination host is read from `docs/_config.yml` rather than typed; a generated stub carries the destination as canonical, meta refresh **and** script, keeping `#fragment` and query; and `site_url()` **raises** rather than defaulting when `url:` is absent | the site moved off github.io because that host is blocked on the readership's network, so the old address keeps serving forwarding pages instead of 404s -- and that set drifts **silently in the direction that matters**. Add a document and the real site grows a page; the forwarding site does not, and a reader on an old link gets a 404 from a host that looks like it is working, with nothing reporting it: the new site is fine, the old site is fine, and the one path between them is missing. Two failures are pinned that both LOOK like success. A generator walking an empty tree writes an empty forwarding site and exits 0. A generator defaulting on a missing `url:` writes a complete site forwarding everywhere to nothing -- every stub renders, every link is clickable, and every one is wrong. The meta refresh alone drops the `#fragment`, so a deep link into a section would land at the page top, which is why the script route is pinned separately and the case asserts `location.hash` |
